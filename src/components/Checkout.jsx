      ...
      drop-shadow-2xl text-white" >
        ...
        {cart.map((item, i) => (
            <div  key={i} className="mb-5 mt-5">
                ...
                <div className="ml-96 inline">Quantity:
                <select className="  bg-black focus:outline-none" onChange={(e) => editQuantity(item,e.target.value)}>
                ...
                <p onClick={()=> removeItem(item)} className="inline ml-4 text-xl cursor-pointer">&#10006;</p>
                
                <h2>{item.price}€</h2>
                {item.size && <h3 className="mt-4 text-sm">Size: {item.size}</h3>}
